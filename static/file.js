function get_local_text(e, cache = 0) {
    let cur = 0;
    let ctn = '';
    if (cache) ctn = e;
    else ctn = e.target.result;
    //read content
    let n = ctn.length;
    let a = '';
...
function import_local(str = 'fileInput', cache = 0) {
    const fileInput = document.getElementById(str);
    let file = '';
    if (cache) {
        get_local_text(localStorage.getItem('distantmarch'), 1);
        change_page('menu1');
    } else {
