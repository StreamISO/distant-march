...
function change_page(page, m_page = -1) {
    ...
    into('import_local', lastoption == 0, option == 0);
    into('save_local', lastoption == 0, option == 0);
    into('auto_save', lastoption == 0, option == 0);
...
function calc_velocity() {
    if (velocity.low_get() >= 3.00e8) return;
    let pp_vel = velocity;
    ...
    do {
        ...
        if (pp_vel.low_get() >= 3.00e8) pp_vel.to(3.00, 8);
...
function print_global() {
    document.getElementById('auto_save_val').innerHTML = auto_save ? '是' : '否';
